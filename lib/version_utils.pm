...
Returns true if called on openeuler
=cut

sub is_openeuler {
    return 1 if check_var('DISTRI', 'openeuler');
    return 0;
}
...
sub uses_qa_net_hardware {
    return !check_var("IPXE", "1") && is_ipmi || (check_var("BACKEND", "generalhw") && !check_var("ARCH", "riscv64"));
}

=head2 uses_riscv_hardware

Returns true if the SUT uses riscv hardware
=cut

sub uses_riscv_hardware {
    return check_var("BACKEND", "generalhw") && check_var("ARCH", "riscv64");
}
