...
sub ensure_unlocked_desktop {
    ...
    wait_still_screen($wait_time);
    while ($counter--) {
        my @tags = qw(displaymanager-oerv displaymanager-password-prompt-riscv generic-desktop-oerv screenlock screenlock-password authentication-required-user-settings authentication-required-modify-system guest-disabled-display oh-no-something-has-gone-wrong);
        ...
        if (match_has_tag 'displaymanager-oerv') {
        ...
        if ((match_has_tag 'displaymanager-password-prompt-riscv') || (match_has_tag 'screenlock-password')) {
            if ($password ne '') {
                type_password;
                # poo#97556
                if (check_var('DESKTOP', 'minimalx')) {
        ...
        if (match_has_tag 'generic-desktop-oerv') {
            send_key 'esc';
            unless (get_var('DESKTOP', '') =~ m/awesome|enlightenment|lxqt/) {
                ...
                if (check_screen 'desktop-runner-oerv', 30) {
                    send_key 'esc';
...
sub handle_login {
    ...
    send_key_until_needlematch('displaymanager-oerv', $mykey, 31, 3);
    if (get_var('ROOTONLY')) {
        wait_screen_change {assert_and_click('displaymanager-oerv')};
        # we now use this tag to support login as root
        if (check_screen 'displaymanager-username-notlisted-oerv', 10) {
            record_info 'bgo#731320/boo#1047262 "not listed" Login screen for root user is not intuitive';
            assert_and_click 'displaymanager-username-notlisted-oerv';
            wait_still_screen 3;
        }
        enter_cmd "root";
    }
    ...
    assert_screen 'displaymanager-password-prompt-oerv';
    type_password($mypwd);
    send_key 'ret';
    wait_still_screen;
    ...
    assert_screen([qw(generic-desktop-oerv gnome-activities)], 180);
    if (match_has_tag('gnome-activities')) {
        send_key_until_needlematch [qw(generic-desktop-oerv language-change-required-update-folder)], 'esc';
        if (match_has_tag('language-change-required-update-folder')) {
            assert_and_click('reserve_old_folder_name');
            assert_screen([qw(generic-desktop-oerv)]);
        }
    }
}
