...
sub handle_displaymanager_login {
    my ($self, %args) = @_;
    assert_screen [qw(displaymanager-oerv emergency-shell emergency-mode)], $args{ready_time};
...
sub wait_boot_past_bootloader {
    ...
    elsif ( is_generalhw && check_var("VIDEOMODE","vnc") ) {
        select_console('sut');
    }
    ...
    my @tags = qw(generic-desktop-oerv emergency-shell emergency-mode);
    ...
    return 1 if match_has_tag('generic-desktop-oerv');
    # the last check after previous intervals must be fatal
    assert_screen \@tags, $check_interval;
    handle_emergency_if_needed;

    handle_broken_autologin_boo1102563() if match_has_tag('displaymanager-oerv');
