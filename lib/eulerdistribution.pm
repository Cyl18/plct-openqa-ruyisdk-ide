...
use version_utils qw(is_hyperv_in_gui is_sle is_leap is_svirt_except_s390x is_tumbleweed is_openeuler);
use x11utils qw(desktop_runner_hotkey ensure_unlocked_desktop);
use x11test qw(open_terminal_here);
use Utils::Backends;
...
use testapi qw(send_key %cmd assert_screen check_screen check_var click_lastmatch get_var save_screenshot
  ...
  wait_still_screen wait_screen_change get_required_var diag hashed_string assert_script_run assert_and_click);
...
sub handle_password_prompt {
    ...
    if (is_serial_terminal()) {
        ...
        assert_screen("password-prompt-oerv", 60);
    }
...
sub init_desktop_runner {
    ...
    mouse_hide(1);
    if (!check_screen('desktop-runner-oerv', $timeout)) {
        record_info('workaround', "desktop-runner-oerv does not show up on $hotkey, retrying up to three times (see bsc#978027)");
        send_key 'esc';    # To avoid failing needle on missing 'alt' key - poo#20608
        send_key_until_needlematch 'desktop-runner-oerv', $hotkey, 4, 10;
    }
    wait_still_screen(5);
...
sub ensure_installed {
    ...
    x11test::open_terminal_here;
    $self->become_root;
    ensure_serialdev_permissions;
    if (check_var("VIDEOMODE", "vnc")) {
        type_string("dnf install -y $pkglist\n");
    }
    else {
        assert_script_run ("dnf install -y $pkglist", 300);
    }
    wait_still_screen 1;
    send_key("alt-f4");    # close terminal
    assert_and_click("close-terminal-window");
    assert_screen 'generic-desktop-oerv' if is_openeuler;
}
...
sub init_consoles {
    ...
    if (get_var('BACKEND', '') =~ /ikvm|ipmi|spvm|pvm_hmc|generalhw/) {
        $self->add_console(
            'root-ssh',
            'ssh-xterm',
            {
...
sub activate_console {
    ...
    if ($type eq 'console') {
        ...
        if (get_var('BACKEND', '') =~ /ipmi|s390x|spvm|pvm_hmc/ || get_var('S390_ZKVM')) {
            ...
            my @tags = ("tty$nr-selected-oerv-2203", "text-logged-in-$user-oerv-2203");
            ...
            # Wait a bit to avoid false match on 'text-logged-in-$user-oerv-2203', if tty has not switched yet,
            ...
            if (match_has_tag("tty$nr-selected-oerv-2203")) {
                enter_cmd "$user";
                handle_password_prompt;
            }
        ...
        assert_screen "text-logged-in-$user-oerv-2203", $waittime;
        ...
        assert_screen("text-logged-in-$user-oerv-2203", 60);
        ...
        handle_password_prompt;
        ensure_user($user);
        assert_screen(["text-logged-in-$user-oerv-2203", "text-login"], 60);
        $self->set_standard_prompt($user, skip_set_standard_prompt => $args{skip_set_standard_prompt});
